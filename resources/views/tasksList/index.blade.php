                    ...
                    @auth
                        <div class="flex">
                            ...
                            <div class="flex-auto text-right mt-2">
                                <a href="/task/create" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Add new Task</a>
                            </div>
                        </div>
                        @if(auth()->user()->tasks)
                                ...
                                <tbody>
                                    @foreach ( auth()->user()->tasks as $task)
                                        <tr class="border-b hover:bg-orange-100">
                                            ...
                                            <td class="p-3 px-5">
                                                <a href="/task/{{$task->id}}/edit" name="edit" class="mr-3 text-sm bg-blue-500 hover:bg-blue-700 text-white py-1 px-2 rounded focus:outline-none focus:shadow-outline">Edit</a>
                                                <form action="/task/{{$task->id}}" method="post" class="inline-block">
                                                    @csrf
                                                    @method('DELETE')
